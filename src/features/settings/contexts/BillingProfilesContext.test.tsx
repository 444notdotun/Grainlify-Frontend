...
describe('BillingProfilesContext', () => {
  ...
  describe('addProfile', () => {
    ...
    it('persists the new profile to localStorage', () => {
      const { result } = renderHook(() => useBillingProfiles(), { wrapper })
      act(() => {
        result.current.addProfile(makeProfile({ id: 5, name: 'Persisted' }))
      })
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '[]')
  ...
  describe('updateProfile', () => {
    it('merges updates into the matching profile', () => {
      const { result } = renderHook(() => useBillingProfiles(), { wrapper })
      act(() => {
        result.current.addProfile(makeProfile({ id: 1, name: 'Original' }))
      })
      act(() => {
        result.current.updateProfile(1, { name: 'Updated', city: 'Berlin' })
      })
    ...
    it('does not mutate other profiles', () => {
      ...
      act(() => {
        result.current.updateProfile(1, { name: 'Changed' })
      })
      expect(result.current.profiles[1].name).toBe('Second')
    })
    ...
    it('is a no-op when the id does not exist', () => {
      const { result } = renderHook(() => useBillingProfiles(), { wrapper })
      act(() => {
        result.current.addProfile(makeProfile({ id: 1 }))
      })
      act(() => {
        result.current.updateProfile(999, { name: 'Ghost' })
      })
      expect(result.current.profiles).toHaveLength(1)
      expect(result.current.profiles[0].id).toBe(1)
    })
    ...
    it('persists the updated profile to localStorage', () => {
      const { result } = renderHook(() => useBillingProfiles(), { wrapper })
      act(() => {
        result.current.addProfile(makeProfile({ id: 1, name: 'Before' }))
      })
      act(() => {
        result.current.updateProfile(1, { name: 'After' })
      })
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '[]')
  ...
  describe('setProfiles', () => {
    it('replaces the full profile list', () => {
      ...
      act(() => {
        result.current.setProfiles([makeProfile({ id: 2, name: 'Replaced' })])
      })
      expect(result.current.profiles).toHaveLength(1)
      expect(result.current.profiles[0].id).toBe(2)
    })
    ...
    it('persists the replacement list to localStorage', () => {
      const { result } = renderHook(() => useBillingProfiles(), { wrapper })
      act(() => {
        result.current.setProfiles([makeProfile({ id: 10, name: 'Only' })])
      })
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '[]')
      expect(stored).toHaveLength(1)
    ...
    it('accepts an empty array to clear all profiles', () => {
      const { result } = renderHook(() => useBillingProfiles(), { wrapper })
      act(() => {
        result.current.addProfile(makeProfile())
      })
      act(() => {
